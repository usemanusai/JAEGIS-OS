...
# Import security scanner (assuming it's in the same directory or PYTHONPATH)
try:
    from security_scanner import SecurityScanner, SecurityViolation, SecurityThreatLevel
    SECURITY_SCANNER_AVAILABLE = True
except ImportError:
    print("Warning: Security scanner not available. Running without security scanning.")
...
class eJAEGISAutoSync:
    def __init__(self, jaegis_path: Path, monitor_interval: int = DEFAULT_MONITOR_INTERVAL):
        ...
        self.excluded_patterns = {
            ...
            '*.log', '*.tmp', '*.temp', '.env', '.env.*', 'security_violations.log'
        }
    ...
    def scan_file_security(self, file_path: str, content: str) -> bool:
        """Scan file for security violations. Returns True if safe to upload."""
        if not self.security_enabled:
            return True  # Security scanning disabled

        try:
            violations = self.security_scanner.scan_file_content(content, file_path)

            if violations:
                should_block, reason = self.security_scanner.should_block_upload(violations)

                if should_block:
                    self.logger.error(f'SECURITY BLOCK: {file_path} - {reason}')

                    # Log critical violations
                    critical_violations = [v for v in violations if v.threat_level == SecurityThreatLevel.CRITICAL]
                    if critical_violations:
                        self.logger.error(f'Critical violations in {file_path}:')
                        for violation in critical_violations[:3]:  # Log first 3
                            self.logger.error(f'  Line {violation.line_number}: {violation.pattern_name}')

                    return False
                else:
                    # Log warnings but allow upload
                    self.logger.warning(f'Security warnings for {file_path}: {len(violations)} violations (allowing upload)')

            return True

        except Exception as e:
            self.logger.warning(f'Security scan failed for {file_path}: {e}')
            # Fail-safe: allow upload if scanner fails
            return True

    def upload_file_to_branch(self, file_path: str, branch_name: str, commit_message: str) -> bool:
        """Upload file to specific branch with security scanning"""
        try:
            ...
            try:
                ...
                # Binary file - skip security scan
                content_text = None
                self.logger.info(f'Binary file detected, skipping security scan: {file_path}')

            # Perform security scan on text content
            if content_text is not None:
                if not self.scan_file_security(file_path, content_text):
                    self.logger.error(f'Upload blocked by security scan: {file_path}')
                    return False
