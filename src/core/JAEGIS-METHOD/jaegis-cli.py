...
# Import common configuration management
try:
    from core.config_management import JAEGISConfigManager, JAEGISIntelligentConfig
...
class eJAEGISConfig:
    """eJAEGIS configuration management - now using common library"""

    def __init__(self, eJAEGIS_dir: Path):
        self.eJAEGIS_dir = eJAEGIS_dir

        # Use common library if available, otherwise fallback to legacy implementation
        if COMMON_CONFIG_AVAILABLE:
            self._config_manager = JAEGISConfigManager(eJAEGIS_dir)
        else:
    ...
    def load_user_config(self):
        ...
        if COMMON_CONFIG_AVAILABLE:
            return self._config_manager.load_user_config()
        else:
            # Legacy fallback
            try:
    ...
    def load_project_config(self):
        ...
        if COMMON_CONFIG_AVAILABLE:
            return self._config_manager.load_project_config()
        else:
            # Legacy fallback
            try:
    ...
    def save_user_config(self, config):
        ...
        if COMMON_CONFIG_AVAILABLE:
            return self._config_manager.save_user_config(config)
        else:
            # Legacy fallback
            try:
